...
function New_InputCandy() {
    ...
    GetSignal: function ( player_number, button_id ) {
			var device=__INPUTCANDY.players[player_number-1].device;
			if ( device == none or device >= array_length(__INPUTCANDY.devices) ) return null;
			var len=array_length(__INPUTCANDY.states[device].signals);
			for ( var i=0; i<len; i++ )
			 if ( __INPUTCANDY.states[device].signals[i].button == button_id)
			   return __INPUTCANDY.states[device].signals[i]
			return null;
		},
    ...
    AxisToLength: function ( H, V ) { return point_distance(0, 0, H, V); },
