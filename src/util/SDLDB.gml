...
function __SDLDB_Load_Start() {
 __INPUTCANDY.SDLDB_Buffer="";
 __INPUTCANDY.SDLDB_Load_Done=!file_exists("SDLDB.txt")
...
function SDLDB_Load_Start() {
 ...
 __INPUTCANDY.SDLDB_File=file_text_open_read("SDLDB.txt");
 __INPUTCANDY.SDLDB_Size_Bytes=0;
 __INPUTCANDY.SDLDB_Read_Bytes=0;
}


function __SDLDB_Load_Step() {
...
function SDLDB_Load_Step() {
	if ( __INPUTCANDY.SDLDB_Load_Done || __INPUTCANDY.SDLDB_File == -1) {
		__INPUTCANDY.SDLDB_Load_Done = true
		return true
	}
	
	var idx = 0
	while (!file_text_eof(__INPUTCANDY.SDLDB_File)) {
		if (idx > 32) {
			break	
		}
		
		var line = file_text_read_string(__INPUTCANDY.SDLDB_File)
		__INPUTCANDY.SDLDB_Buffer = __INPUTCANDY.SDLDB_Buffer + line + "\n"
		__INPUTCANDY.SDLDB_Read_Bytes += 1
		file_text_readln(__INPUTCANDY.SDLDB_File)
		idx += 1
	}
	
	if (!file_text_eof(__INPUTCANDY.SDLDB_File)) {
		return false	
	}
	
	file_text_close(__INPUTCANDY.SDLDB_File)
	__INPUTCANDY.SDLDB_Load_Done = true
